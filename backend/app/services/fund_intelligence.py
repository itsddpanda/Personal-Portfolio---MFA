...
def fetch_fund_intelligence(isin: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"DaaS returned 503 Processing for ISIN {isin}. Retry in {retry_after}s.")
            ...
            logger.error(f"DaaS API Error ({response.status_code}) for ISIN {isin}: {response.text}")
            return None
