...
from app.services.mfapi_client import fetch_scheme_data, extract_metadata, extract_nav_history
...
def get_scheme_details(amfi_code: str, x_user_id: str = Header(None), session: Session = Depends(get_session)):
    if not x_user_id:
    ...
    if not scheme.fund_house or not scheme.scheme_category:
        ...
        if mfapi_data:
            ...
            history_count = session.exec(select(NavHistory).where(NavHistory.scheme_id == scheme.id)).all()
    ...
    transactions = session.exec(
        ...
        .join(Portfolio)
        .where(
            Transaction.scheme_id == scheme.id,
    ...
    inflow_types = ["PURCHASE", "PURCHASE_SIP", "SIP", "SWITCH_IN", "STP_IN", "OPENING_BALANCE", "DIVIDEND_REINVESTMENT"]
    ...
    return {
        "scheme": {
            ...
            "latest_nav_date": scheme.latest_nav_date.isoformat() if scheme.latest_nav_date else None,
        },
        "kpis": {
