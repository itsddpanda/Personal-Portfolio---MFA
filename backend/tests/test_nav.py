...
class TestNavService(unittest.TestCase):
    def setUp(self):
        ...
        # Setup Scheme
        self.scheme = Scheme(isin="INF123456789", name="Test Fund", amfi_code="123456", type="EQUITY")
    ...
    def test_fetch_latest_nav_success(self, mock_get):
        ...
        mock_response.json.return_value = {
            "meta": {"fund_house": "Test House", "scheme_type": "Open Ended", "scheme_category": "Equity", "scheme_code": 123456, "scheme_name": "Test Fund"},
            "data": [{"date": "15-02-2024", "nav": "150.50"}, {"date": "14-02-2024", "nav": "149.00"}],
            "status": "SUCCESS"
        }
    ...
    def test_sync_navs(self, mock_get):
        ...
        # Verify DB Update
        updated_scheme = self.session.exec(select(Scheme).where(Scheme.amfi_code == "123456")).first()
