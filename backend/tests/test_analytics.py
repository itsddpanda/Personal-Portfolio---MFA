...
class TestAnalytics(unittest.TestCase):
    def setUp(self):
        ...
        self.scheme = Scheme(isin="INF123456789", name="Growth Fund", type="EQUITY", latest_nav=120.0)
    ...
    def test_simple_investment(self):
        ...
        t1 = Transaction(
            id="txn1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 1, 1), type="PURCHASE", amount=10000.0, units=100.0, nav=100.0
        )
        self.session.add(t1)
        self.session.commit()
    ...
    def test_sip_xirr(self):
        ...
        self.session.add(Transaction(
            id="sip1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 1, 1), type="SIP", amount=5000.0, units=50.0, nav=100.0
        ))
        
        # SIP 2: July 1, 2023
        self.session.add(Transaction(
            id="sip2", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 7, 1), type="SIP", amount=5500.0, units=50.0, nav=110.0
        ))
        self.session.commit()
    ...
    def test_redemption_impact(self):
        ...
        # Buy 100 units
        self.session.add(Transaction(
            id="buy1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 1, 1), type="PURCHASE", amount=10000.0, units=100.0, nav=100.0
        ))
        
        # Sell 50 units (Half)
        self.session.add(Transaction(
            id="sell1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 6, 1), type="REDEMPTION", amount=6000.0, units=50.0, nav=120.0
        ))
        self.session.commit()
    ...
    def test_fifo_complex_scenario(self):
        ...
        self.session.add(Transaction(
            id="b1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 1, 1), type="PURCHASE", amount=5000.0, units=50.0, nav=100.0
        ))
        
        # Buy 2: 50 units @ 200 (Cost: 10000)
        self.session.add(Transaction(
            id="b2", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 2, 1), type="PURCHASE", amount=10000.0, units=50.0, nav=200.0
        ))
        ...
        self.session.add(Transaction(
            id="s1", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 3, 1), type="REDEMPTION", amount=3600.0, units=30.0, nav=120.0
        ))
        ...
        self.session.add(Transaction(
            id="s2", folio_id=self.folio.id, scheme_id=self.scheme.id,
            date=date(2023, 4, 1), type="REDEMPTION", amount=6000.0, units=40.0, nav=150.0
        ))
