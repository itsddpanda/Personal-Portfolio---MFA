...
def seed_benchmark_data(num_txns=2000):
    with Session(engine) as session:
        ...
        # Create 3 schemes
        schemes = [
            Scheme(isin="INF001", name="Scheme 1", amfi_code="100001", type="EQUITY", latest_nav=100.0),
            Scheme(isin="INF002", name="Scheme 2", amfi_code="100002", type="EQUITY", latest_nav=200.0),
            Scheme(isin="INF003", name="Scheme 3", amfi_code="100003", type="EQUITY", latest_nav=50.0),
        ]
        for s in schemes:
            session.add(s)
        session.commit()
...
def run_manual_verify(user_id):
    ...
    with Session(engine) as session:
        statement = select(Transaction, Scheme).join(Folio).join(Portfolio).join(Scheme).where(Portfolio.user_id == user_id)
...
if __name__ == "__main__":
    ...
    if diff_val < 0.01 and diff_invested < 0.01:
        ...
        print(f"❌ FAILURE: Results mismatch! Value Diff: {diff_val}, Invested Diff: {diff_invested}")
